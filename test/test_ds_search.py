...
api.save_config({"active_platform": "deepseek", "deepseek_search": True})

conv = app.conv_manager.new_conversation()
conv["model"] = "deepseek-chat"
app.conv_manager.save_conversation(conv)

app.conv_manager.add_message(conv["id"], "user", [{"type": "text", "text": "What is the weather in Tokyo today?"}])
app.current_conv = app.conv_manager.load_conversation(conv["id"])

print("Sending message with search enabled...")
res = api.send_message(conv["id"], "What is the weather in Tokyo today?", [])
